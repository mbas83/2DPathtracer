...
namespace detail
{
	...
	// extension not in original gli
	inline gli::format gl::findEx(gl::internal_format InternalFormat, gl::external_format* DstExternalFormat, gl::type_format* DstType)
	{
		for (int FormatIndex = FORMAT_FIRST; FormatIndex <= FORMAT_LAST; ++FormatIndex)
		{
			std::size_t const Index = FormatIndex - FORMAT_FIRST;
			...
			if(DstExternalFormat) *DstExternalFormat = this->FormatDesc[Index].External;
			if(DstType) *DstType = this->FormatDesc[Index].Type;
